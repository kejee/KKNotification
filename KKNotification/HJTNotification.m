...
@interface HJTNotification()

@property (nonatomic,weak) id<HJTNotificationDelegate> delegate;

@end
@implementation HJTNotification
static HJTNotification *noti;
+ (HJTNotification *)shareNitifacation{
    
    if (noti == nil) {
        noti = [[HJTNotification alloc]init];
    }
    return noti;
...
+ (void)HJT_notificationWithTitle:(NSString *)title msg:(NSString *)msg{
    ...
    noti_window.backgroundColor = [UIColor colorWithRed:0.0 green:0.670 blue:0.988 alpha:1.00];//[UIColor colorWithRed:0.180 green:0.125 blue:0.051 alpha:1.00];
    ...
    noti_Btn.backgroundColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.800];//[UIColor colorWithRed:0.489 green:0.350 blue:0.251 alpha:1.000];
...
+ (void)tap:(UIGestureRecognizer*)tap{
    ...
    if([[[self class] shareNitifacation].delegate respondsToSelector:@selector(HJTNotificationDidClickWithMsg:)]){
        UILabel *label = [noti_window viewWithTag:10];
        [[[self class] shareNitifacation].delegate HJTNotificationDidClickWithMsg:label.text];
    }
    if ([[[self class] shareNitifacation].delegate respondsToSelector:@selector(HJTNotificationClickWithKey:)]) {
        
        [[[self class] shareNitifacation].delegate HJTNotificationClickWithKey:[[self class] shareNitifacation].clicKey];
    }
    
    [[self class] dismiss];
}
+(void)setClicKey:(NSString *)clicKey{
    [[self class] shareNitifacation].clicKey = clicKey;
}
